...
set.seed(123)
#Model 1
Data_RF300 <- Data2[,c(1,4+1,4+1+16,4+1+32,4+1+48,4+1+64, 4+6,4+6+16,4+6+32,4+6+48,4+6+64, 4+7,4+7+16,4+7+32,4+7+48,4+7+64,4+9,4+9+16,4+9+32,4+9+48,4+9+64,4+10,4+10+16,4+10+32,4+10+48,4+10+64,
...
set.seed(222)
#Change number of trees as needed
rf <- randomForest(Category~.,data=train, ntree=1000,mtry=6, method = "gbm",
                   importance=TRUE,
                   proximity=TRUE)
print(rf)
attributes(rf)
...
aa<- data.frame(Prediction_result)
aa
aas<-B
aas$Prediction <- Prediction_result
table(aas$Prediction)
write.table(aas, file="Prediction Output RF8000 RCP6_trial_3cats_model2_boost.csv", sep=",", row.names = FALSE, col.names = TRUE)
